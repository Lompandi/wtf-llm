...
def _prompt(spec: InputSpec, harness: HarnessSpec, example: str) -> str:
    ...
    return (
        ...
        f"`InputAddress ResolveInputAddress(P, size_t Bytes)` for P of either uint64_t or "
        f"Gva_t. InputAddress converts implicitly to BOTH uint64_t and Gva_t, so assign it "
        f"to whichever you want. Everywhere else Gva_t's constructor is EXPLICIT: wrap "
        f"with Gva_t(...), add with `Addr + Gva_t(sizeof(x))`, and never compare a Gva_t "
        f"against 0.\n"
